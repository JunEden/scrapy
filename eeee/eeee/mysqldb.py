...
class database(Save):
	...
	def _Save__createTable(self,name):
		with self.__connection.cursor() as cursor:
			sql = 'create table %s(jobname text,jobLink text,company text,companyAddress text,companyLink text,jobArea text,experience text,school text,description text, salary text);' %name
			#最好加個try知道中間發生什麼錯誤
			try:
				cursor.execute(sql)
