...
from pchomeEnum import InputType
from pchomedaoFactory import factory
import logging
...
class MYSQLPipeline:
	...
	def process_item(self,item,spider):
		...
		if not result:
			raise DropItem("MYSQL: %s is duplicated"%item['Productname'])
		return item
	def __del__(self):
		self.db.close()
...
class MONGOPipeline:
	...
	def process_item(self,item,spider):
		result = self.db.insert(item)
		if not result:
			raise DropItem("Mongo: %s is duplicated"%item['Productname'])
		return item
	def __del__(self):
		self.db.close()
...
class CSVPipeline:
	...
	def process_item(self,item,spider):
		result = self.db.insert(item)
		if not result:
			raise DropItem("CSV: %s is duplicated"%item['Productname'])
		return item
