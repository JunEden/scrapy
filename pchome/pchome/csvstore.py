...
#檢查存不存在的方法 用os 裡的listdir去取
	...
	def _Save__createTable(self,name):
		obj = open(os.path.join(self.__path,name['Brand']+".csv"),"a+")
	...
	def _Save__deDuplicate(self,item):
		with open(os.path.join(self.__path,item['Brand']+".csv"),"r") as f:
			data = [i.split(",")for i in f.readlines()]

		if [i[0] for i in data if item ['Productname'] in i]:
			return True
		return False
	...
	def insert(self,item):
		if not self._Save__isExist(item['Brand']):
			...
			writer = csv.writer(open(os.path.join(self.__path,item['Brand']+".csv"),"a+"))
